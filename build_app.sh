...
echo -e "${BLUE}=== PushToTalk .app Bundle Builder ===${NC}\n"

# Configuration
APP_NAME="PushToTalk"
BUNDLE_ID="com.pushtotalk.app"
VERSION="1.0.0"
...
if [ ! -f "${BUILD_DIR}/PushToTalkSwift" ]; then
    echo -e "${RED}❌ Build failed: executable not found at ${BUILD_DIR}/PushToTalkSwift${NC}"
    exit 1
fi
...
cp "${BUILD_DIR}/PushToTalkSwift" "${MACOS_DIR}/"
chmod +x "${MACOS_DIR}/PushToTalkSwift"
...
if [ -d "${APP_DIR}" ]; then
    ...
    echo "        ├── PkgInfo"
    echo "        ├── MacOS/"
    echo "        │   └── PushToTalkSwift (executable)"
    echo "        └── Resources/"
    ...
    echo "  Executable: PushToTalkSwift"
    ...
    echo "  2. Sign the app: ./sign_app.sh"
    echo "  3. Create DMG: ./create_dmg.sh"
