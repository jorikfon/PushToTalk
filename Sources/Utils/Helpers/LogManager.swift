...
/// Также пишет в файл: ~/Library/Logs/PushToTalk/pushtotalk.log
...
private class FileLogger {
    ...
    private let queue = DispatchQueue(label: "com.pushtotalk.filelogger", qos: .utility)

    private init() {
        // Создаем директорию для логов
        let logsDir = FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent("Library/Logs/PushToTalk")

        try? FileManager.default.createDirectory(at: logsDir, withIntermediateDirectories: true)

        logFileURL = logsDir.appendingPathComponent("pushtotalk.log")
