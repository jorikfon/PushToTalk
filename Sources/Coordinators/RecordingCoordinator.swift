...
public final class RecordingCoordinator {
    ...
    private func performTranscription(audioData: [Float], finalizingSession: Int, submitAfterInsert: Bool) async {
        ...
        // ОСОЗНАННЫЙ компромисс (по явному выбору пользователя): при непрерывной речи
        // без паузы перед отпусканием хвост после последнего превью (превью обновляется
        // по паузе или раз в ~4с) может не попасть во вставку — приоритет нулевой
        // задержки над полнотой. НЕ «исправлять» добавлением финального прохода.
